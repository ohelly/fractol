...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/06/12 17:09:49 by ohelly            #+#    #+#             */
/*   Updated: 2019/06/12 17:09:51 by ohelly           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_iter_burningship(t_fr *t, double pr, double pi)
{
	int		i;
	double	newre;
	double	newim;
	...
	{
		temp = newre;
		newre = fabs(temp * temp - newim * newim + pr);
		newim = fabs(2 * temp * newim + pi);
		i++;
	}
	if (i == t->it_max)
...
void	*burningship_draw(void *thr)
{
	t_fr		*t;
	double		pr;
	double		pi;
...
int		burningship_thread(t_fr *fr)
{
	...
	while (++i < NUM_THREADS)
	{
		...
		pthread_create(&p[i], NULL, burningship_draw, (void*)&t[i]);
	}
	while (i--)
